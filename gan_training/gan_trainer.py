...
class GanFasterRCNNTrainer(nn.Module):
    ...
    * :obj:`total_loss`: The sum of 4 loss above.
    ...
    def __init__(self, faster_rcnn):
        super(GanFasterRCNNTrainer, self).__init__()
    ...
    def forward(self, imgs, bboxes, labels, scale):
        ...
        * :math:`R` is the number of bounding boxes per image.
        ...
        # outFromGenerator = self.faster_rcnn.generator(outFromFirstConvBlock, img_size, scale)
        ...
        # #     sample_roi_index)
        #
        # roi_indices = at.totensor(sample_roi_index).float()
        # rois = at.totensor(sample_roi).float()
        # indices_and_rois = t.cat([roi_indices[:, None], rois], dim=1)
        # xy_indices_and_rois = indices_and_rois[:, [0, 2, 1, 4, 3]]
        # indices_and_rois = xy_indices_and_rois.contiguous()

        outFromGenerator, loc, label = self.faster_rcnn.generator(outFromFirstConvBlock, img_size, scale, bbox, label)
        pool, gt_roi_loc, gt_roi_label = self.faster_rcnn.roi(features, roi, bbox, label)
        pool = pool + outFromGenerator
        pool = pool.view(pool.size(0), -1)
        roi_cls_loc, roi_score = self.faster_rcnn.classifier(pool)
